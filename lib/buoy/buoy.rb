module Buoy
	...
	def self.get_caniuse_data
		...
		if !File.exist?(datafile) || File.mtime(datafile) < (Time.now - 604800) # file doesnt exist or is more than a week old
			json = open("http://caniuse.com/jsonp.php").read.gsub(/^\s*\(\s*(.*)\s*\)\s*$/, '\1')
	...
	def self.settings
		JSON.parse(File.read(File.join(Dir.pwd, 'app', 'assets', 'stylesheets', 'buoy-settings.json')))
	end
					...
					agent_data['versions'].each { | version |
						if (version)
							...
							scss = template.
								...
								gsub(self.template_var('buoy-no-markup'), path == 'web-fonts' ? "\n$buoy-no-markup: true;" : '')
